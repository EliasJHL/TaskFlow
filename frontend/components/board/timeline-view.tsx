...
export function TimelineView({ boardId }: TimelineViewProps) {
  ...
  const getTaskStatus = (task: Task) => {
    ...
    if (isBefore(due, now)) {
      return { status: "overdue", color: "text-red-600", bgColor: "bg-red-50 dark:bg-red-950", icon: AlertTriangle }
    } else if (isBefore(due, tomorrow)) {
      return { status: "due-soon", color: "text-orange-600", bgColor: "bg-orange-50 dark:bg-orange-950", icon: Clock }
    } else {
      return { status: "upcoming", color: "text-blue-600", bgColor: "bg-blue-50 dark:bg-blue-950", icon: Calendar }
    }
  }
  ...
  const filterTasksByTime = (tasks: Task[]) => {
    ...
    switch (timeFilter) {
      case "overdue":
        return tasks.filter((task) => task.dueDate && isBefore(new Date(task.dueDate), now))
      case "this-week":
        const weekStart = startOfWeek(now, { weekStartsOn: 1 })
        const weekEnd = endOfWeek(now, { weekStartsOn: 1 })
        return tasks.filter(
          (task) => task.dueDate && isWithinInterval(new Date(task.dueDate), { start: weekStart, end: weekEnd }),
        )
      case "this-month":
        const monthStart = startOfMonth(now)
        const monthEnd = endOfMonth(now)
        return tasks.filter(
          (task) => task.dueDate && isWithinInterval(new Date(task.dueDate), { start: monthStart, end: monthEnd }),
        )
      case "upcoming":
        return tasks.filter((task) => task.dueDate && isAfter(new Date(task.dueDate), now))
      default:
        return tasks
    }
  }
  ...
  const filteredTasks = filterTasksByStatus(filterTasksByTime(boardTasks)).sort((a, b) => {
        ...
        {filteredTasks.length === 0 ? (
          ...
          filteredTasks.map((task) => {
            ...
            return (
              <Card
                key={task.id}
                className={`cursor-pointer transition-all hover:shadow-md ${taskStatus?.bgColor || ""}`}
                    ...
                    <div className="flex flex-col items-center min-w-[80px]">
                      ...
                      <div className="text-center mt-2">
                        ...
                        <p className="text-xs text-muted-foreground">
                          {format(new Date(task.dueDate!), "yyyy", { locale: fr })}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex-1 space-y-2">
                      <div className="flex items-start justify-between gap-2">
                        <div className="flex-1">
                          <h4 className="font-medium text-balance">{task.title}</h4>
                          {task.description && (
                      ...
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {column && (
                          <Badge variant="outline" style={{ borderColor: column.color }}>
                            {column.title}
                          </Badge>
                        )}
