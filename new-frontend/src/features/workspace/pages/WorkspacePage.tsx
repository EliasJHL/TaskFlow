...
export const WorkspacePage = () => {
  ...
  if (error || !data?.workspace) return <div className="p-10 text-red-500">Impossible de charger le workspace.</div>;
        ...
        <div className="mb-8">
          ...
          <div className="flex items-center gap-2 text-sm text-muted-foreground mb-6">
             ...
             <span className="text-foreground font-medium flex items-center gap-2">
                <div className="w-2 h-2 rounded-full" style={{ backgroundColor: workspaceColor }} />
                {workspace.name}
             </span>
          </div>
          ...
          <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6">
            ...
            <div className="flex gap-6">
              <div className="h-24 w-24 rounded-2xl shadow-2xl flex items-center justify-center text-4xl font-bold text-white shrink-0 ring-4 ring-background" style={{ backgroundColor: workspaceColor }}>
        ...
        <Tabs defaultValue="boards" className="w-full mt-12">
          <div className="border-b border-border mb-8">
             <TabsList className="h-auto p-0 bg-transparent space-x-6">
                <CustomTabTrigger value="boards" icon={KanbanSquare} label="Tableaux" count={workspace.boards?.length} />
                <CustomTabTrigger value="views" icon={CalendarIcon} label="Vues" />
                <CustomTabTrigger value="members" icon={Users} label="Membres" count={workspace.members.length} />
                <CustomTabTrigger value="settings" icon={Filter} label="Filtres" />
             </TabsList>
          </div>

          <TabsContent value="boards" className="space-y-6 animate-in slide-in-from-bottom-4 duration-500 fade-in">
            {workspace.boards && workspace.boards.length > 0 ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                <button className="group relative flex flex-col items-center justify-center h-32 rounded-xl border-2 border-dashed border-muted hover:border-primary/50 hover:bg-primary/5 transition-all duration-300">
                   ...
                   <span className="text-sm font-medium text-muted-foreground group-hover:text-primary">Créer un tableau</span>
                </button>
